...
namespace VendreEcommerce\EloquentMysqli\Connections;
...
class MySQLiConnection extends Connection implements ConnectionInterface
{
    ...
    public function select($query, $bindings = [], $useRead = true)
    {
        return $this->run($query, $bindings, function ($query, $bindings) use ($useRead) {
            ...
            $preparedStatement = $this->getMySqliForSelect($useRead)
                ->prepare($query);
            ...
            $statement = $this->prepared2($preparedStatement);
    ...
    public function cursor($query, $bindings = [], $useReadPdo = true)
    {
        $statement = $this->run($query, $bindings, function ($query, $bindings) use ($useReadPdo) {
            ...
            $preparedStatement = $this->getMySqliForSelect($useReadPdo)
                ->prepare($query);
        ...
        // This means we are in pretending mode
        if ($this->pretending()) {
            yield $statement;
        } else {
            // @var \mysqli_result
     ...
     * This trick will make Eloquent calls to PDO to use MySQLi instead.
     *
     * @return \mysqli
     */
     ...
     * Tricking into setting MySQLi instead.
     *
     * @param  \mysqli|null  $mysqli
     * @return $this
     */
